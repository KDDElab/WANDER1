...
class EasyMKL(OneStepMKL):
    ...
    def _combine_kernels(self):
        ...
        ker_matrix = (1-self.lam) * self.func_form(self.KL).cpu() + self.lam * identity_kernel(n_sample)
        ...
        weights = torch.tensor([(yg.view(n_sample, 1).T @ K.cpu() @ yg).item() for K in self.KL])
        weights = weights / weights.sum()

        ker_matrix = self.func_form(self.KL, weights)
        bias = 0.5 * (gamma @ ker_matrix.cpu() @ yg).item()
        return Solution(
            weights     = weights,
            objective   = None,
